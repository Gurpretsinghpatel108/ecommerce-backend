...
app.use(
  cors({
    origin: [
      FRONTEND_URL,              // Dynamic frontend from .env
      "http://localhost:5173",   // React Admin
      "http://localhost:8081",   // EXPO WEB
      "http://192.168.29.72:8081", // EXPO MOBILE (same network)
      "*"                        // NGROK / TEMP
...
app.post("/api/products", upload.single("image"), async (req, res) => {
  try {
    const newProduct = new Product({ ...req.body, image: req.file ? req.file.filename : null });
    await newProduct.save();
    const populated = await Product.findById(newProduct._id).populate("categoryId", "name").populate("subcategoryId", "name");
    io.emit("productUpdated", populated);
    res.status(201).json({ success: true, data: populated });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
...
app.put("/api/products/:id", upload.single("image"), async (req, res) => {
  try {
    ...
    const populated = await Product.findById(updatedProduct._id).populate("categoryId", "name").populate("subcategoryId", "name");
    io.emit("productUpdated", populated);
    res.json({ success: true, message: "Product updated", data: populated });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
...
app.post("/api/profiles", upload.single("profilePicture"), async (req, res) => {
  try {
    const profile = new Profile({ ...req.body, profilePicture: req.file ? req.file.filename : null });
    await profile.save();
    io.emit("profileUpdated", profile);
    res.status(201).json({ success: true, data: profile });
  } catch (err) {
